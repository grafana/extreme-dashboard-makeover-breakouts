...
7. It's hard to see the title. This come from the data. In this case, we don't want to change the data. Let's imagine we can't change the table from the query. We will transform the data:
    - On the lower left side, click on *Transformations*
    - Search for and select *Organize fields by name*
    - Hide the fields by clicking on the small eye close to the name of the field. Hide all except *Value* and *container*
